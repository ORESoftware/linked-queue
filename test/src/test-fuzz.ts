...
function createNotifyObj(uuid: string, pid: number = Math.floor(Math.random() * 10000), ttl: number = 5000): NotifyObj {
    return {
...
function testHeadTail(q: LinkedQueue<NotifyObj, string>): void {
...
function runFuzzTest(versionName: string, LinkedQueueClass: typeof LinkedQueue): void {
    const q = new LinkedQueueClass<NotifyObj, string>();
    ...
    const operationsMap: {[key: number]: () => void} = {
        0: () => {
            // removeAll
            q.removeAll();
            existingKeys.clear();
            ...
            if (q.length > 0) {
                ...
                results.forEach(item => {
                    if (Array.isArray(item) && !IsVoid.check(item[0])) {
                        existingKeys.delete(item[0] as string);
                    }
                });
        ...
        15: () => {
            ...
            for (const [key, value] of q.dequeueIterator()) {
                if (!IsVoid.check(key)) {
                    count++;
                    existingKeys.delete(key as string);
                }
            }
...
try {
    runFuzzTest('Latest (2.1.129)', LinkedQueue);
