...
function runTests(versionName, LinkedQueueClass) {
    ...
    test('AddToFront items', () => {
        ...
        assert.strictEqual(q.length, 2);
        const first = q.first();
        assert.ok(!linked_queue_1.IsVoid.check(first[0]));
    ...
    test('Dequeue items', () => {
        ...
        assert.ok(!linked_queue_1.IsVoid.check(dequeued[0]));
    ...
    test('Remove by key', () => {
        ...
        assert.ok(!linked_queue_1.IsVoid.check(removed[0]));
    ...
    test('Remove and addToFront pattern', () => {
        ...
        assert.ok(!linked_queue_1.IsVoid.check(alreadyAdded[0]));
        q.remove('uuid1');
    ...
    test('Deq with more items than available', () => {
        ...
        const validItems = dequeued.filter(item => !linked_queue_1.IsVoid.check(item[0]));
    ...
    test('Large number of operations', () => {
        ...
        const validRemaining = remaining.filter(item => !linked_queue_1.IsVoid.check(item[0]));
...
const summary = {};
try {
    const results = runTests('Latest (2.1.129)', linked_queue_1.LinkedQueue);
